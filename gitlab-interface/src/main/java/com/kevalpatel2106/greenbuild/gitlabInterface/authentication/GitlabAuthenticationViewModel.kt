...
import com.kevalpatel2106.greenbuild.gitlabInterface.Constants
import com.kevalpatel2106.greenbuild.gitlabInterface.GitlabServerInterface
 ...
 */
internal class GitlabAuthenticationViewModel @Inject constructor(
    ...
    fun prepareApiUrl(serverUrl: String): String {
        if (serverUrl.contains(application.getString(R.string.schema_https))) {
            return serverUrl.replace(
                    ...
                    newValue = "${application.getString(R.string.schema_https)}gitlab."
            ).plus("/api/v4")
        }
        return ""
    }
    ...
    fun getProfileUrl(apiUrl: String): String? {
        return when (apiUrl) {
            Constants.GITLAB_API -> application.getString(R.string.profile_url_gitlab_com)
    ...
    fun validateAuthToken(accessToken: String, apiUrl: String) {
        ...
        with(GitlabServerInterface.get(application = application, baseUrl = apiUrl, accessToken = accessToken)) {

            if (this == null) {
                //Invalid base URL format.
                //This should never happen!!!
                throw IllegalStateException("The url ($apiUrl) is not in gitlab CI api format. " +
                        "See https://docs.gitlab.com/ee/api/README.html#basic-usage.")
            } else {
                this.getMyAccount()
